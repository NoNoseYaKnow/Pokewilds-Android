...
| PokeWilds Bug Fix Patcher | Divinakra's v1.0 source at commit `93f633a`; ASM 9.7 artifacts checked against the hashes in `game-patches/build.sh` | Source-built `bugfix.jar` agent in the APK; enabled fixes run against the separately acquired, unchanged official game JAR | Vendored source, MIT license, and ASM license are under `game-patches/`; the built agent includes the applicable notices. The floor fix has a local lossless map-save extension documented in `game-patches/PROVENANCE.md`. |
| Radial wheel and zoom proposal | User-provided `PokeWilds-v0.8.11-RadialWheel-ZoomControls.patch` in the project folder | A reduced, source-built `control-patches.jar` agent in the APK; enabled features run against the separately acquired, unchanged official game JAR | `control-patches/PROVENANCE.md` records the adapted code and excluded input replacement and art assets. Redistribution terms for this proposal have not been established. |
